...
const showCredential = (credential) => {
  ...
  if(vcData) {
      document.querySelector(".credential-title").textContent = "Employee Credential have been successfully verified!"
      document.querySelector(".credential-subtitle").textContent = "You have obtained 20% off in the Shop"
...
const requestPresentation = async () => {
    ...
    if(/Android|iPad|iPhone|iPod/i.test(navigator.userAgent)) {
      document.querySelector('#url').href = presentationResponse.url;
